...
Character::Character(CharacterClass charClass, int str, int agi, int end)
    : characterClass(charClass), strength(str), agility(agi), endurance(end),
      level(1), turnCount(0), weapon("Кулаки", 1, DamageType::CRUSHING) {
  ...
  for (const auto &[cls, lvl] : classLevels) {
    ...
    case CharacterClass::WARRIOR:
      ...
      if (lvl >= 1 && turnCount == 1) {
        ...
        std::cout << "Порыв к действию! +" << weapon.getDamage() << " урона."
                  << std::endl;
      }
      break;
    case CharacterClass::BARBARIAN:
...
bool Character::attack(Monster &target) {
  ...
  if (!hitResult) {
    ...
    if (target.getPoison()) {
      if (getClassLevels().count(CharacterClass::ROGUE) > 0 &&
          getClassLevels().at(CharacterClass::ROGUE) == 3 &&
          target.getPoison()->getPoisonStatus()) {
        int damage = target.getPoison()->getDamage();
        std::cout << "Но яд всё равно наносит "
                  << target.getPoison()->getDamage() << " урона!" << std::endl;
  ...
  /*if (target.getPoison()) {
    std::cout << "текущий урон от яда = " << target.getPoison()->getDamage()
              << "." << std::endl;
  }*/
  return hitResult;
}
...
void Character::takeDamage(int damage, const Monster &attacker) {
  ...
  if (classLevels.count(CharacterClass::WARRIOR) &&
      classLevels.at(CharacterClass::WARRIOR) >= 2 &&
      strength > attacker.getStrength()) {
    finalDamage -= 3;
    if (finalDamage < 0)
      finalDamage = 0;
    std::cout << "Щит воина блокирует 3 урона!" << std::endl;
  } else if (classLevels.count(CharacterClass::BARBARIAN) &&
             classLevels.at(CharacterClass::BARBARIAN) >= 2) {
    finalDamage -= endurance;
    if (finalDamage < 0)
      finalDamage = 0;
    std::cout << "Каменная кожа снижает урона на " << endurance << "!"
              << std::endl;
  }
  currentHealth -= finalDamage;
  std::cout << "Получено урона = " << finalDamage << ". Здоровье "
            << currentHealth << "/" << fullHealth << "." << std::endl;
}

void Character::healFull() { currentHealth = fullHealth; }
...
void Character::levelUp(CharacterClass newClass) {
  ...
  calculateStats();
  healFull();
  std::cout << "Уровень повышен! Теперь вы " << getClassName()
            << ", ваш уровень - " << level << std::endl;
}

void Character::equip(const Weapon &dropWeapon) {
  weapon = dropWeapon;
  std::cout << "Экипировано новое оружие: " << weapon.getName()
            << " (урон = " << weapon.getDamage() << ")" << std::endl;
}
...
void Character::displayStats() {
  ...
  std::cout << "Здоровье: " << currentHealth << "/" << fullHealth << "."
            << std::endl;
