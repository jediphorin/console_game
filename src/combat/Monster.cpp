...
Monster::Monster(MonsterType type)
    : monsterType(type), turnCount(0),
      rewardWeapon("Нет", 0, DamageType::SLASHING) {
  switch (type) {
...
void Monster::applySpecialEffects(int &damage, DamageType damageType,
  ...
  case MonsterType::GOLEM:
    ...
    if (damage < 0)
      damage = 0;
    std::cout << "Каменная кожа снижает урон на " << endurance << "!"
              << std::endl;
    break;
  case MonsterType::SLIME:
  ...
  if (attacker.getClassLevels().count(CharacterClass::ROGUE) > 0 &&
      attacker.getClassLevels().at(CharacterClass::ROGUE) == 3) {
    if (!poison) {
...
void Monster::takeDamage(int damage, DamageType damageType,
                         const Character &attacker) {
  ...
  if (poison) {
    ...
    if (poison->getPoisonStatus()) {
      std::cout << "Яд забрал " << poison->getDamage() << " единиц здоровья."
                << std::endl;
      finalDamage += poison->getDamage();
      // std::cout << "УРОН ОТ ЯДА ДОБАВЛЕН ВО ВЕСЬ УРОН 127" << std::endl;
    } else {
      std::cout << "Яд будет отравлять монстра со следующего хода."
                << std::endl;
  ...
  if (finalDamage > 0) {
    ...
    std::cout << name << " получает урон = " << finalDamage << ". Здоровье "
              << currentHealth << "/" << fullHealth << "." << std::endl;
  } else
    std::cout << name << " не получает урона!" << std::endl;
}

void Monster::swallowPoison(int damage) { currentHealth -= damage; }

std::string Monster::getName() const { return name; }
int Monster::getBaseDamage() const { return baseDamage; }
int Monster::getStrength() const { return strength; }
int Monster::getAgility() const { return agility; }
int Monster::getEndurance() const { return endurance; }
int Monster::getFullHealth() const { return fullHealth; }
int Monster::getCurrentHealth() const { return currentHealth; }
Weapon Monster::getRewardWeapon() const { return rewardWeapon; }
Poison *Monster::getPoison() const { return poison.get(); }